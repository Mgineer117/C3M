...
model_W, model_Wbot, model_u_w1, model_u_w2, W_func, u_func = get_model(
    num_dim_x, num_dim_control, w_lb=args.w_lb, use_cuda=args.use_cuda
)
...
def sample_x(xref):
    ...
    """
    xe = (XE_MAX - XE_MIN) * np.random.rand(
        num_dim_x, 1
    ) + XE_MIN  # Generate random perturbation
...
def forward(x, xref, uref, _lambda, verbose=False, acc=False, detach=False):
    ...
    if acc:
        return (
            loss,
            ((L >= 0).sum(dim=1) == 0).cpu().detach().numpy(),
            ((L >= 0).sum(dim=1) == 0).cpu().detach().numpy(),
...
optimizer = torch.optim.Adam(
    ...
    + list(model_u_w2.parameters()),
    lr=args.learning_rate,
)


def trainval(
    X, bs=args.bs, train=True, _lambda=args._lambda, acc=False, detach=False
):  # trainval a set of x
